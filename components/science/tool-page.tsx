...
export function Panel({ title, children }: { title: string; children: React.ReactNode }) { return <Reveal as="section" immediate className="rounded-2xl border border-border bg-card p-5 shadow-sm transition-colors hover:border-primary/30"><h2 className="font-mono text-xs uppercase tracking-[0.16em] text-muted-foreground">{title}</h2><div className="mt-5">{children}</div></Reveal> }
...
export function MolecularTool({ bonding = false }: { bonding?: boolean }) {
  ...
  async function loadVerified(searchQuery = query) {
    const normalizedQuery = searchQuery.trim()
    if (!normalizedQuery) { setError("Enter a molecule name or formula."); return }
    setLoading(true); setError(""); setQuery(normalizedQuery)
    try { const response = await fetch(`/api/pubchem?q=${encodeURIComponent(normalizedQuery)}`); const data = await response.json(); if (!response.ok) throw new Error(data.error); setSelected(data) }
    catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to load structure") }
    finally { setLoading(false) }
  }
  const extent = selected.atoms.reduce((max, atom) => Math.max(max, Math.abs(atom.x), Math.abs(atom.y), Math.abs(atom.z)), 1)
  const cameraDistance = Math.max(6, extent * 3.2)
