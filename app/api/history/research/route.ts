...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [wikiResponse, geoResponse, wikidataResponse] = await Promise.all([
      ...
      fetch(`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(query)}&language=en&format=json&limit=5`, { headers, next: { revalidate: 3600 } }),
    ])
    const wiki = wikiResponse.ok ? await wikiResponse.json() : null
    const wikidata = wikidataResponse.ok ? await wikidataResponse.json() : null
